classdef fp_nouns   
    methods (Static)        
        function [ParOrden,ang] = calcula_parametro_orden(index,desde,hasta,OSC)
        ...
        function [paramorden,cis] = integro_palabras_comunidad(C,Trends,init,indc,contrend)
            t0=init.t0;
            tspan=init.tspan;
            years=init.years;
            desde=init.desde;
        ...
        %     figure(indc);clf;hold all
            ...
            for indp=1:Ncom
                ...
                if contrend==1
                    trend=1000*Trends(ind_palabra,desde:hasta);%trend real
                else
...
%                 times=ones(1,length(y0));
                %calculo fases:
%                 trend=reshape(Trends(indp).tr,1,length(years));
%                 y0norm=y0-trend;
                phaseexp(indp,:)=angle(hilbert(y0));
...
%             size_com(indc)=Ncom;            
        ...
        function C=integro_comunidades(T,F_TIMELINE,init,amplitud,contrend,mincom)
...
%                 fprintf('\t%3d %3d %2.2f\t%2.1f%%\t%2.0fs/%2.0fs\n',indc,Ncom,mean(paramorden),porcentaje,toc,toc*100/porcentaje)
        ...
        function C=integro_comunidades_parord(T,F_TIMELINE,init,amplitud,contrend,mincom,language)
            ...
            for indc=1:length(C)
                ...
                C(indc).cis=cis;
                %C(indc).phaseexp=phaseexp;
                C(indc).par=paramorden;

                porcentaje=100*sum([C(1:indc).n_palabras])/sum([C.n_palabras]);
                fprintf('\t%3d %3d %s-%i \t%2.1f%%\t%2.0fs/%2.0fs\n',indc,Ncom,language,contrend,porcentaje,toc,toc*100/porcentaje)
                drawnow
            end            
